...
def balanced(m):
    ...
    Lw = total_weight(L)
    Rw = total_weight(R)
    return Lw * length(L) == Rw * length(R)
    '''
    if is_planet(m):
        return True
    else:
        ...
        return balanced(left_end) and balanced(
            right_end) and torque_left == torque_right
...
def replace_leaf(t, find_value, replace_value):
    ...
    return tree(
        label(t),
        [replace_leaf(m, find_value, replace_value) for m in branches(t)])
...
def preorder(t):
    ...
    help(t)
    # Error: return [label(t)]+[preorder(x) for x in branches(t)]
    return re
    '''
    # Alternate solution
    if not branches(t):
...
def interval(a, b):
    assert a <= b, """Construct an interval from a to b."""
    return [a, b]
