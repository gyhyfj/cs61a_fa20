class VendingMachine:
    ...
    def vend(self):
        ...
        if self.money < self.price:
            return 'You must add ${} more funds.'.format(self.price -
                                                         self.money)
            ...
            temp = self.money - self.price
            self.money = 0
            self.stock -= 1
            return 'Here is your {} and ${} change.'.format(self.product, temp)
    '''

    def vend(self):
        if self.stock == 0:
            return 'Inventory empty. Restocking required.'
        diff = self.money - self.price
        if diff < 0:
            return 'You must add ${} more funds.'.format(-diff)
        message = 'Here is your {}'.format(self.product)
        if diff > 0:
            message += ' and ${0} change'.format(diff)
        self.money = 0
        self.stock -= 1
        return message + '.'

    def add_funds(self, addfunds):
        if self.stock == 0:
            return 'Inventory empty. Restocking required. Here is your ${}.'.format(
                addfunds)
...
class Coin:
    ...
    def worth(self):
        ...
        if Mint.current_year - self.year > 50:
            exvalue = Mint.current_year - self.year - 50
        return self.cents + exvalue
...
def store_digits(n):
    ...
    re_val = Link(digits[i])
    temp = re_val
    while i > 0:
        i -= 1
        temp.rest = Link(digits[i])
        temp = temp.rest
    return re_val
    '''
    result = Link.empty
...
def is_bst(t):
    ...
    if len(t.branches) == 1:
        ...
        return valid_branches and bst_max(c1) <= t.label and bst_min(
            c2) > t.label
    else:
        return False
